...
def dbs(Request) :
	...
	if 'userdb' in Request.session:
		a = user.objects.get(email = Request.session['useremail'])
		db = userdbs.objects.get(email = a, dbtype = Request.session['userdb'], dbname = Request.session['dbname'])
		# if db.dbtype == 'mysql':
...
def plot(Request):
	if Request.method == 'POST':
		...
		###    DATABASES
		a = user.objects.get(email = useremail)
		b = userdbs.objects.get(email = a, dbtype = userdb, dbname = Request.session['dbname'])
...
def getcolumns(Request) :
	if Request.method == 'POST':
		...
		###    DATABASES
		a = user.objects.get(email = useremail)
		b = userdbs.objects.get(email = a, dbtype = userdb, dbname = dbname)
		if userdb == 'mysql':
